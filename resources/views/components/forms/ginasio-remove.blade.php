<div class="flex flex-col w-full h-full pt-40 bg-gray-600/50" @click="idmodal=null">
    <div class="flex flex-col justify-center w-1/2 shadow dark:bg-gray-700 h-auto m-5 p-3 bg-white self-center rounded-md">
        ...
        <div class="p-2 mb-2 border-b-2 border-gray-300 ">
                <h1 class="text-2xl">{{ $ginasio->nome }}</h1>
        </div>
        <p>{{ $ginasio->telefone }}</p>
        <ul>
            <li>CEP: {{ $ginasio->cep }}</li>
            {{-- <li>Preço: {{ $ginasio->preco }}</li> --}}
            <li>Estacionamento: {{ $ginasio->estacionamento ? 'Sim' : 'Não' }}</li>
        </ul>
        <form id="{{$ginasio->id_ginasio}}"
             wire:submit.prevent=
                "remove({{ $ginasio->id_ginasio }})">
        ...
        <table>
            <tr align="center">
                ...
                <td>
                    <x-button-error @click="idmodal=null;" form="{{$ginasio->id_ginasio}}" type="submit"
